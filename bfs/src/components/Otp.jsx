...
import { setCountdown, setIncorrectOtpWarning, setIsResendDisabled, setIsVerifingLoader, setOtp, setOtpScreen } from '@/features/auth/authSlice'
...
const Otp = () => {
    ...
    const incorrectOtpWarning = useSelector((state) => state.auth.incorrectOtpWarning)
    ...
    return (
        ...
        <div className=' flex flex-col justify-center items-center h-dvh'>
            <div onClick={() => dispatch(setOtpScreen(false))} className='max-w-6 max-h-6 absolute top-5 left-5'>
                <LeftArrow />
            </div>
            ...
            <div className='relative mx-[2.1875rem] mt-[1.375rem] '>
                ...
                {
                    countdown > 0 ?
                        (<div className='flex justify-between mt-5 relative'>          {incorrectOtpWarning ?
                            (<div className=' min-w-[8.5rem] min-h-5 flex items-center font-lato font-normal text-sm text-[#E21931]' ><Alert className='mr-2' /> OTP is incorect :/</div>) : (<div></div>)
                        }
                            {
                                isVerifingLoader ?
                                    (<p className='font-inria font-normal text-sm text-[#999999] min-h-5'>Verifying ...</p>)
                                    :
                                    (
                                        <p className='font-inria font-normal text-sm text-[#999999] min-w-[6.75rem] min-h-5'>Resend in {countdown} sec</p>
                                    )
                            }
                        </div>)
                        ...
                        (<div className='flex justify-between mt-5 relative'>
                            ...
                            }
                            {
                                isVerifingLoader ?
                                    ...
                                    :
                                    (
                                        <button onClick={resendHandle} className='font-inria font-normal text-sm min-w-[3.125rem] min-h-5 text-[#286C11]'>Resend</button>
                                    )
                            }
            ...
            <button ref={verifyButton} onClick={handleSubmit} className={`mt-[6.375rem] min-w-[18.75rem] min-h-[3.125rem] font-inria font-bold text-base text-[#286C11] border-[0.1rem] border-[#286C11] rounded-[1.5625rem] ${otp.join('').length == 6 ? 'bg-[#286C11] text-[#F0F0F5]' : 'bg-white'}  ${isVerifingLoader ? 'invisible' : 'visible'}`}>Verify</button>
